...
layout(location = 4) in float oow;
layout(location = 5) in flat uint tmu[GLIDE_NUM_TMU];
layout(location = 8) in flat uint c;
...
layout(std430, set = 3, binding = 0) uniform PipelineConfig {
    uint ccombine_func;
    uint ccombine_f;
    uint ccombine_local;
    uint ccombine_other;
